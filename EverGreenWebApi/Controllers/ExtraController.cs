...
namespace EverGreenWebApi.Controllers
{
    ...
    {
        static readonly IExtraRepository _repository = new ExtraRepository();

        [HttpPost]
        public HttpResponseMessage GetAllExtraCharges(ExtraModel model)
        {
            ResponseStatus response = new ResponseStatus();
            try
            {
                var data = _repository.GetAllExtraCharges(model.StoreId);
                if (data.Count() > 0)
                {
